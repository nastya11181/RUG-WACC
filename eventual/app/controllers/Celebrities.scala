...
object Celebrities extends Controller with MongoController {
  ...
  def create() = Action.async(parse.json) { request =>
    ...
    val celebrity = Celebrity(Option(BSONObjectID.generate), name) // create the celebrity
  ...
  def update(id: String) = Action.async(parse.json) { request =>
    ...
    collection.update(BSONDocument("_id" -> objectID), modifier).map(
      _ => Ok(Json.toJson(Celebrity(Option(objectID), name)))) // return the modified celebrity in a JSON
  }
